...
typedef struct{
	volatile uint32_t NVIC_ISER[8];		/* Registers enable interrupts, and show which interrupts are enabled. 	*/
	volatile uint32_t Reserved1[24];
	volatile uint32_t NVIC_ICER[8];		/* Registers disable interrupts, and show which interrupts are enabled. */
	volatile uint32_t Reserved2[24];
	volatile uint32_t NVIC_ISPR[8];		/* Registers force interrupts into the pending state, and show which interrupts are pending 	*/
	volatile uint32_t Reserved3[24];
	volatile uint32_t NVIC_ICPR[8];		/* Registers remove the pending state from interrupts, and show which interrupts are pending. */
	volatile uint32_t Reserved4[24];
	volatile uint32_t NVIC_IABR[8];		/* Registers indicate which interrupts are active. 						*/
	volatile uint32_t Reserved5[24];
	volatile uint8_t  NVIC_IPR [240];	/* Used to enable a specific interrupt IRQ 		*/
	volatile uint32_t Reserved6[644];
	volatile uint32_t NVIC_STIR;		/* Used to enable a specific interrupt IRQ 		*/
